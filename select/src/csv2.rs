...
// impl<T> CSV for djanco::Iter<T> where T: CSVItem {
//     fn to_csv(self, location: impl Into<String>) -> Result<(), std::io::Error> {
//         unimplemented!()
//     }
// }
...
impl WithStaticNames for ProjectId   { fn names() -> Vec<&'static str> { vec!["project_id"] } }
impl WithStaticNames for CommitId    { fn names() -> Vec<&'static str> { vec!["commit_id"]  } }
impl WithStaticNames for UserId      { fn names() -> Vec<&'static str> { vec!["user_id"]    } }
impl WithStaticNames for PathId      { fn names() -> Vec<&'static str> { vec!["path_id"]    } }
...
impl WithStaticNames for Commit { fn names() -> Vec<&'static str> { unimplemented!() } }
impl WithStaticNames for User   { fn names() -> Vec<&'static str> { unimplemented!() } }
impl WithStaticNames for Path   { fn names() -> Vec<&'static str> { unimplemented!() } }
...
impl WithNames for ProjectId     { fn names(&self) -> Vec<String> { to_owned_vec!(<Self as WithStaticNames>::names()) } }
impl WithNames for UserId        { fn names(&self) -> Vec<String> { to_owned_vec!(<Self as WithStaticNames>::names()) } }
impl WithNames for PathId        { fn names(&self) -> Vec<String> { to_owned_vec!(<Self as WithStaticNames>::names()) } }
impl WithNames for CommitId      { fn names(&self) -> Vec<String> { to_owned_vec!(<Self as WithStaticNames>::names()) } }
...
impl<T> WithNames for Vec<T> where T: WithNames + WithStaticNames {
    fn names(&self) -> Vec<String> {
        match self.first() {
            Some(e) => e.names(),
            None => to_owned_vec!(<T as WithStaticNames>::names()),
        }
    }
}
...
//         self.peekable().peek().map(names().join(","))
//     }
// }
