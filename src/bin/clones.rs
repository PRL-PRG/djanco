    ...
    */    
    ...
    projects_all(&config, &log, &database).map(|project| {
        return (
            ...
            project.subscriber_count().unwrap_or(0),
            project.size().unwrap_or(0),
            project.commit_count().unwrap_or(0),
            project.license().unwrap_or_else(|| String::new()),
            project.language().unwrap_or(Language::Other),
            project.major_language().unwrap_or(Language::Other),
        );        
    }).into_csv_with_headers_in_dir(
